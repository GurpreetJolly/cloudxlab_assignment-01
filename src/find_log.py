...
def guess (lower, upper):
    """Computes an estimate of the log base10 of a number between lower and upper.
    Args:
    ...
    if abs(mid_log10 - target) < tolerance:
        print(f"Estimated log10 of {target} is approximately {mid}")
        return
    elif mid_log10 < target:
        guess(mid, upper)
    else:
        guess(lower, mid)
...
target = input("Enter an integer more than zero to find the log10 of: ")
try:
    if len(target) > 18:
